...
#
# Author：MTimer - LNMPA For !!**CentOS/RedHat***!! Inspired by NewEraCracker Server For Windows
#
...
# If you are customer of MTimer CMS, more packages will be installed
echo -n "Are u a customer of MTimer CMS ? [1]Yes or [2]No ?  "
read CUSTOMER
if [ "$CUSTOMER" = "1" ]; then
	CUSTOMER=yes
else
...
# Server location
echo -n "[1]China or [2]World wide ? "
read WHERE
if [ "$WHERE" = "1" ]; then
	WHERE=china
else
...
# Auto fdisk
echo -n "Auto fdisk [1]Yes or [2]No ? "
read AUTOFDISK
if [ "$AUTOFDISK" = "1" ]; then
...
if [ -e /mtimer/www/${mtimer_dir} ]; then
	...
	if [ "$CUSTOMER" = "yes" ];then
		echo '  MTimerCMS: /mtimer/www/${mtimer_dir}/';
	fi
	if [ "$WEBTYPE" = "nginx" ];then
